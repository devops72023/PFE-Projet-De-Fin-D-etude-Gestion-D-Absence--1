<?php
    ...
    if(isset($_POST)){
        ...
        if(!isCurrentUser($conn, 'nomProf', $nom)){
            mysqli_query($conn, "UPDATE professeurs
                                        SET nomProf='$nom'
                                        WHERE codeProf='$codeProf'") or die(mysqli_error($conn));
            
        }
        if(!isCurrentUser($conn, 'prenomProf', $prenom)){
            mysqli_query($conn, "UPDATE professeurs
                                        SET prenomProf='$prenom'
                                        WHERE codeProf='$codeProf'") or die(mysqli_error($conn));
        }
        if(!isCurrentUser($conn, 'email', $email)){
            mysqli_query($conn, "UPDATE professeurs
                                        SET email='$email'
                                        WHERE codeProf='$codeProf'") or die(mysqli_error($conn));
        }
        if(!isCurrentUser($conn, 'telephone', $telephone)){
            mysqli_query($conn, "UPDATE professeurs
                                        SET telephone='$telephone'
                                        WHERE codeProf='$codeProf'") or die(mysqli_error($conn));
        }
        if(!isCurrentUser($conn, 'genre', $gender)){
            mysqli_query($conn, "UPDATE professeurs
                                        SET genre='$gender'
                                        WHERE codeProf='$codeProf'") or die(mysqli_error($conn));
        }
